...
export default function FacilitiesList() {
  ...
  const [rooms, setRooms] = useState<FacilitiesInterface[]>([]);
  ...
  const getRooms = async () => {
    try {
      ...
      const reRenderFacilities = data.data.rooms.map(
        (fac: FacilitiesInterface) => ({
          ...fac,
          datafac: { id: fac._id, name: fac.name },
        })
      );
      setRooms(reRenderFacilities);
      // console.log(local);
    } catch (error) {
      console.log(error);
    }
          ...
          <Box sx={style}>
            <Box display={"flex"} justifyContent={"space-between"}>
              <Typography
                ...
                component="h2"
              >
                {isUpdate ? `Update this ${roomName}` : " Add Facility"}
              </Typography>
              <HighlightOff
                sx={{ cursor: "pointer" }}
                onClick={handleClose}
